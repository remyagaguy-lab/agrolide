...
export default async function MesFormationsPage() {
  ...
  const getStatusBadge = (statut: string | null) => {
    if (statut === 'en_cours') {
      return <span className="bg-[#f0f7f0] text-[#1b5e38] border border-[#c3dec4] px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 uppercase tracking-wider"><CheckCircle className="w-3 h-3" /> En cours</span>
    }
    switch (statut) {
      case 'inscrit':
    ...
    <div className="max-w-[1200px] mx-auto px-2 md:px-4 py-6 space-y-6">
      ...
      {data.length === 0 ? (
        ...
        <div className="space-y-6">
          {data.map((ins) => {
            const isExternal = !!ins.formation;
            const formation = isExternal ? ins.formation : ins.sessions_formation?.formation;
            ...
            return (
              <div key={ins.id} className="bg-white border border-[#e8e8e4] rounded-2xl shadow-sm hover:border-[#1b5e38] transition-colors p-5 flex flex-col md:flex-row gap-6 items-start md:items-center group">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                      {isExternal ? formation.source_externe || 'Externe' : formation.modalite}
                    </span>
                  </div>
                  ...
                  {isExternal && formation.thematique && (
                    <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                      <div className="flex items-center">
                ...
                <div className="w-full md:w-auto md:min-w-[180px] flex flex-col gap-2 border-t md:border-t-0 md:border-l border-[#e8e8e4] pt-4 md:pt-0 md:pl-5">
                  {!isExternal && ins.statut === 'en_attente_paiement' && (
                  ...
                  {!isExternal && ins.statut === 'complete' && profile && profile.nom && profile.prenom && sessionData && (
                  ...
                  {!isExternal && ins.statut === 'inscrit' && (
                    <div className="text-center text-sm text-gray-500 font-medium">
                      En attente de la formation
                    </div>
                  )}

                  {isExternal && formation.lien_externe && (
                    <a 
                      href={formation.lien_externe} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="w-full px-4 py-2 bg-[#1b5e38] text-white text-center rounded-lg text-xs font-bold hover:bg-[#144a2c] transition-colors shadow-sm"
                    >
